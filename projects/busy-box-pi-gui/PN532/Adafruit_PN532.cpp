...
#ifdef PN532DEBUG
		log.println("Firmware doesn't match!");
#endif
		return 0;
	}
...
bool Adafruit_PN532::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout) {
	...
	if (!readack()) {
#ifdef PN532DEBUG
		log.println("No ACK frame received!");
#endif
		return false;
	}
...
#ifdef PN532DEBUG
	log.print("Writing P3 GPIO: "); log.println(pn532_packetbuffer[1], HEX);
#endif
...
uint8_t Adafruit_PN532::readGPIO(void) {
	...
	log.println();
	log.print("P3 GPIO: 0x"); log.println(pn532_packetbuffer[p3offset], HEX);
	log.print("P7 GPIO: 0x"); log.println(pn532_packetbuffer[p3offset + 1], HEX);
	log.print("IO GPIO: 0x"); log.println(pn532_packetbuffer[p3offset + 2], HEX);
	// Note: You can use the IO GPIO value to detect the serial bus being used
	switch (pn532_packetbuffer[p3offset + 2])
	{
	case 0x00:    // Using UART
		log.println("Using UART (IO = 0x00)");
		break;
	case 0x01:    // Using I2C
		log.println("Using I2C (IO = 0x01)");
		break;
	case 0x02:    // Using SPI
		log.println("Using SPI (IO = 0x02)");
		break;
	}
#endif
...
#ifdef MIFAREDEBUG
	log.print("Setting MxRtyPassiveActivation to "); log.print(maxRetries, DEC); log.println(" ");
#endif
...
bool Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
	...
	if (!_usingSPI) {
#ifdef PN532DEBUG
		log.println("Waiting for IRQ (indicates card presence)");
#endif
		if (!waitready(timeout)) {
#ifdef PN532DEBUG
			log.println("IRQ Timeout");
#endif
			return 0x0;
		}
	}
...
#ifdef MIFAREDEBUG
	log.print("Found "); log.print(pn532_packetbuffer[7], DEC); log.println(" tags");
#endif
...
#ifdef MIFAREDEBUG
	log.print("ATQA: 0x");  log.println(sens_res, HEX);
	log.print("SAK: 0x");  log.println(pn532_packetbuffer[11], HEX);
#endif
...
#ifdef MIFAREDEBUG
		log.print(" 0x"); log.print(uid[i], HEX);
#endif
	}
#ifdef MIFAREDEBUG
	log.println();
...
bool Adafruit_PN532::inDataExchange(uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) {
...
#ifdef PN532DEBUG
		log.println("APDU length too long for packet buffer");
#endif
		return false;
	}
	uint8_t i;
...
#ifdef PN532DEBUG
		log.println("Could not send APDU");
#endif
		return false;
	}
	...
	if (!waitready(1000)) {
#ifdef PN532DEBUG
		log.println("Response never received for APDU...");
#endif
		return false;
	}
	...
	if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff) {
...
#ifdef PN532DEBUG
			log.println("Length check invalid");
			log.println(length, HEX);
			log.println((~length) + 1, HEX);
#endif
			return false;
		}
		if (pn532_packetbuffer[5] == PN532_PN532TOHOST && pn532_packetbuffer[6] == PN532_RESPONSE_INDATAEXCHANGE) {
...
#ifdef PN532DEBUG
				log.println("Status code indicates an error");
#endif
				return false;
			}
			...
			*responseLength = length;
			...
			log.print("Don't know how to handle this command: ");
			log.println(pn532_packetbuffer[6], HEX);
			return false;
		}
	}
	else {
		log.println("Preamble missing");
		return false;
	}
}
...
bool Adafruit_PN532::inListPassiveTarget() {
...
#ifdef PN532DEBUG
	log.print("About to inList passive target");
#endif

	if (!sendCommandCheckAck(pn532_packetbuffer, 3, 1000)) {
#ifdef PN532DEBUG
		log.println("Could not send inlist message");
#endif
		return false;
	}
	...
	if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff) {
		...
		if (pn532_packetbuffer[5] == PN532_PN532TOHOST && pn532_packetbuffer[6] == PN532_RESPONSE_INLISTPASSIVETARGET) {
			if (pn532_packetbuffer[7] != 1) {
#ifdef PN532DEBUG
				log.println("Unhandled number of targets inlisted");
#endif
				log.println("Number of tags inlisted:");
				log.println(pn532_packetbuffer[7]);
				return false;
			}

			_inListedTag = pn532_packetbuffer[8];
			log.print("Tag number: ");
			log.println(_inListedTag);

			return true;
		}
		else {
#ifdef PN532DEBUG
			log.print("Unexpected response to inlist passive host");
#endif
			return false;
		}
	}
...
uint8_t Adafruit_PN532::mifareclassic_AuthenticateBlock(uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
...
#ifdef MIFAREDEBUG
	log.print("Trying to authenticate card ");
	Adafruit_PN532::PrintHex(_uid, _uidLen);
	log.print("Using authentication KEY "); log.print(keyNumber ? 'B' : 'A'); log.print(": ");
	...
	{
#ifdef PN532DEBUG
		log.print("Authentification failed: ");
...
{
#ifdef MIFAREDEBUG
	log.print("Trying to read 16 bytes from block "); log.println(blockNumber);
#endif
	...
	{
#ifdef MIFAREDEBUG
		log.println("Failed to receive ACK for read command");
#endif
		return 0;
	}
...
{
#ifdef MIFAREDEBUG
	log.print("Trying to write 16 bytes to block "); log.println(blockNumber);
#endif
	...
	{
#ifdef MIFAREDEBUG
		log.println("Failed to receive ACK for write command");
#endif
		return 0;
	}
	delay(10);
...
uint8_t Adafruit_PN532::mifareultralight_ReadPage(uint8_t page, uint8_t * buffer)
{
	...
	{
#ifdef MIFAREDEBUG
		log.println("Page value out of range");
#endif
		return 0;
	}

#ifdef MIFAREDEBUG
	log.print("Reading page "); log.println(page);
#endif
	...
	{
#ifdef MIFAREDEBUG
		log.println("Unexpected response reading block: ");
		Adafruit_PN532::PrintHexChar(pn532_packetbuffer, 26);
#endif
		return 0;
	}
	...
	/* Display data for debug if requested */
#ifdef MIFAREDEBUG
	log.print("Page "); log.print(page); log.println(":");
...
#ifdef MIFAREDEBUG
	log.print("Trying to write 4 byte page"); log.println(page);
#endif
	...
	if (_usingSPI) {
	  ...
	  // SPI command write.
...
#ifdef PN532DEBUG
		log.print(" 0x"); log.print(PN532_PREAMBLE, HEX);
		log.print(" 0x"); log.print(PN532_PREAMBLE, HEX);
		log.print(" 0x"); log.print(PN532_STARTCODE2, HEX);
		log.print(" 0x"); log.print(cmdlen, HEX);
		log.print(" 0x"); log.print(~cmdlen + 1, HEX);
		log.print(" 0x"); log.print(PN532_HOSTTOPN532, HEX);
#endif
		...
		for (uint8_t i = 0; i < cmdlen - 1; i++) {
			...
			checksum += cmd[i];
#ifdef PN532DEBUG
			log.print(" 0x"); log.print(cmd[i], HEX);
#endif
		}
...
#ifdef PN532DEBUG
		log.print(" 0x"); log.print(~checksum, HEX);
		log.print(" 0x"); log.print(PN532_POSTAMBLE, HEX);
		log.println();
#endif
	}
	else {
